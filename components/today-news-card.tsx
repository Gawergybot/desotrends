...
export function TodayNewsCard({ topics }: { topics: TrendingTopic[] }) {
  return (
    <section className="card p-4" aria-label="Today's News">
      <h2 className="mb-3 text-lg font-semibold">Today&apos;s News</h2>
      <div className="space-y-2">
        {topics.map((topic) => (
          <Link key={topic.slug} href={`/topic/${topic.slug}`} className="block rounded-lg border border-border p-3 hover:bg-slate-900">
            <div className="line-clamp-1 text-sm font-semibold">{topic.title}</div>
